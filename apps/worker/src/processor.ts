...
/**
 * Speech laid over a film the user already has.
 *
 * Nothing is generated by the video model here: the film exists, the text is
 * spoken, and the two are mixed. That is why it costs what speech costs rather
 * than what a clip costs.
 */
async function processNarration(
  ...
  if (params.type !== "narration") {
    throw new Error(`Narration job ${job.id} has ${params.type} params`);
  }
  if (dependencies.narrate === undefined) {
    throw new Error("This worker cannot narrate: no ffmpeg narrator is wired");
  }

  const inputs = await dependencies.loadInputAssets(job.id, job.userId);
  const film = inputs.find((asset) => asset.role === "source_video");
  if (film === undefined) {
    throw new Error(`Narration job ${job.id} has no film to speak over`);
  }

  const speech = await dependencies.voice.createSpeech({
    req_params: {
      text: job.inputParams.prompt,
      speaker: VOICE_PROFILE.speaker,
      audio_params: {
  ...
  const signed = await dependencies.signAssetUrl(film.storageUrl);
  const source = await dependencies.download(signed);
  const mixed = await dependencies.narrate(
    source.body,
    speech.audio,
    params.duckOriginalTo,
  );
  ...
  // Labelled a film: it is a finished piece in its own right, and the gallery
  // should lead with it exactly as it leads with a chain's cut.
  const completed = await dependencies.completeJobWithAssets(job.id, [
    { type: "video", storageUrl, kind: "film" },
  ]);
...
async function processGeneration(
  ...
  if (job.type === "narration") return processNarration(dependencies, job);
  if (job.type === "model3d") {
