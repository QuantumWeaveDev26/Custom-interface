...
test("cost is never zero even at the smallest settings", () => {
  ...
  assert.equal(creditCostFor({ type: "image", size: "1K" }, nearlyFree), 1);
...
test("image and voice are flat-priced regardless of settings", () => {
  assert.equal(creditCostFor({ type: "image", size: "1K" }, PRICING), 1);
  assert.equal(creditCostFor({ type: "image", size: "4K" }, PRICING), 1);
